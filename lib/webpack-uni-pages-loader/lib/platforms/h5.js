...
const compilerVersion = require('@tphone/core/lib/webpack-uni-pages-loader/package.json')['uni-app'].compilerVersion
  ...
  return pages.map(page => {
    ...
    if (process.env.UNI_USING_NVUE_COMPILER) {
      if (!fs.existsSync(pagePath + '.vue') && fs.existsSync(pagePath + '.nvue')) {
        isNVue = true
      } else if (fs.existsSync(pagePath + '.yjpl')) {
        isYjpl = true
      }
    }
...
const genRegisterPageVueComponentsCode = function (pageComponents) {
  return pageComponents
    .map(({
      ...
      const ext = isNVue ? '.nvue' : isYjpl ? '.yjpl' : '.vue'
