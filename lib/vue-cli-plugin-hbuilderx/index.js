...
if(process.env.UNI_LINK) {
  const moduleAlias = require('module-alias');
  require('module-alias/register')
  const baseUrl = 'node_modules/@tphone-beta/core';
  console.log('baseUrl....', path.resolve(process.cwd(), baseUrl));
...
const uniCliI18n = require('@tphone/core/lib/uni-cli-i18n')
...
module.exports = (api, options) => { // 仅处理 app-plus 相关逻辑
  ...
  if (
    ...
    if (!fs.existsSync(path.resolve(process.env.UNI_HBUILDERX_PLUGINS, 'weapp-tools/lib/index.js'))) {
      console.error(uniCliI18n.__('pluginHbuilderx.plaseHXCompileAppPlatform'))
      process.exit(0)
    }
  }
