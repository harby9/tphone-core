...
function resolvePlugins () {
  ...
  const uniPluginsPkg = fs.readFileSync(path.resolve(process.env.UNI_CLI_CONTEXT, 'node_modules/@tphone-beta/core', 'package.json'), 'utf-8');
  return Object.keys(pkg.devDependencies || {})
    ...
    .map(id => {
      ...
      try {
        const pluginDir = path.resolve(__dirname, '../../', id)
        const pluginPkg = require(pluginDir + '/package.json')
